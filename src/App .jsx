...
async function callClaude(messages, model = "claude-haiku-4-5-20251001") {
  const r = await fetch("/api/claude", {
    ...
    body: JSON.stringify({ system:SYSTEM, messages, model }),
  });
...
function Bot({ onClose }) {
  ...
  async function sendDoc(file) {
    ...
    try {
      ...
      if (!isImage && !isPDF) {
        setErr("ניתן לצרף PDF או תמונה בלבד.");
  ...
  async function startMic() {
    ...
    // פתח stream ושמור אותו פעיל — נדרש על Android
    let stream = null;
    try {
      stream = await navigator.mediaDevices.getUserMedia({ audio: true });
    } catch (e) {
      setErr("נדחה. נסה: Chrome ← ⋮ ← הגדרות ← הגדרות אתר ← מיקרופון ← nifgati.co.il ← אפשר");
      setMic(false);
      return;
    }
    ...
    r.onresult = e => {
      setInp(e.results[0][0].transcript);
      cleanup();
    };

    r.onerror = e => {
      if (e.error === "no-speech") setErr("לא זוהה דיבור — נסה שוב.");
      else setErr(`שגיאה: ${e.error}`);
      cleanup();
    };
      ...
      <div style={{ background:"#0a0f1e",border:"1px solid #1e2d4a",borderRadius:20,width:"100%",maxWidth:560,maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden" }}>
        ...
        <div style={{ background:"#0a0f1e",borderTop:"1px solid #1e2d4a",padding:"12px 16px 14px" }}>
          <div style={{ display:"flex",gap:8 }}>
            ...
            <input ref={fileRef} type="file" accept=".pdf,image/*" style={{ display:"none" }} onChange={e=>{ const f=e.target.files?.[0]; if(f) sendDoc(f); e.target.value=""; }}/>
