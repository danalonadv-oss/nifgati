...
function Bot({ onClose }) {
  ...
  async function startMic() {
    ...
    // בקש הרשאת מיקרופון מפורשת — חשוב למובייל
    try {
      await navigator.mediaDevices.getUserMedia({ audio: true });
    } catch (e) {
      setErr("לא ניתן לגשת למיקרופון. אנא אשר גישה בהגדרות הדפדפן.");
      return;
    }
    ...
    r.onerror = e => {
      if (e.error === "not-allowed") {
        setErr("גישה למיקרופון נדחתה. אנא אשר גישה בהגדרות.");
      } else if (e.error === "no-speech") {
        setErr("לא זוהה דיבור. נסה שוב.");
      } else {
        setErr("שגיאת מיקרופון. נסה שוב.");
      }
      setMic(false);
    };
    ...
    try {
      ...
      setErr("לא ניתן להפעיל מיקרופון. נסה שוב.");
    }
  }
      ...
      {!cookie && (
        <div className="ck" role="dialog" aria-modal="false" aria-label="הסכמה לעוגיות">
          <p style={{ maxWidth:700,margin:0 }}>
            האתר משתמש בעוגיות <strong>לצרכי פרסום ממוקד (Remarketing) בלבד</strong>. שיחות הבוט <strong>אינן נשמרות ואינן מתועדות</strong> — לא אצלנו ולא בשום מקום אחר.{" "}
