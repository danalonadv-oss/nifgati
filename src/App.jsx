...
אחרי החישוב: "${MY_NAME}, ${MY_TITLE}, מזמין אותך לייעוץ חינמי ללא התחייבות."`;
...
function Bot({ onClose }) {
  ...
  function startMic() {
    ...
    if(!SR){ setErr("זיהוי קול זמין ב-Chrome בלבד."); return; }
    const r=new SR(); r.lang="he-IL";
      ...
      <div style={{ background:"#0a0f1e",border:"1px solid #1e2d4a",borderRadius:20,width:"100%",maxWidth:560,maxHeight:"90vh",display:"flex",flexDirection:"column",overflow:"hidden" }}>
        ...
        <div style={{ background:"#0a0f1e",borderTop:"1px solid #1e2d4a",padding:"12px 16px 14px" }}>
          <div style={{ display:"flex",gap:8 }}>
            ...
            <input aria-label="הקלד הודעה" style={{ flex:1,background:"#141b2d",border:"1px solid #1e2d4a",borderRadius:12,color:"#fff",fontFamily:"inherit",fontSize:14,padding:"0 14px",outline:"none",height:48,direction:"rtl" }} placeholder="כתוב כאן או לחץ על המיקרופון..." value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>{ if(e.key==="Enter"){e.preventDefault();send(inp);} }}/>
          ...
          <p style={{ textAlign:"center",fontSize:11,color:"#2a3545",marginTop:8 }}>הערכה ראשונית בלבד • אינה מהווה ייעוץ משפטי • השיחה לא נשמרת בשרתינו</p>
        </div>
      </div>
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily:"'Heebo',sans-serif",direction:"rtl",background:"#080d18",color:"#e8edf2",overflowX:"hidden" }}>
      ...
      <header role="banner" style={{ position:"fixed",top:0,right:0,left:0,zIndex:100,background:scrolled?"#080d18f0":"transparent",backdropFilter:scrolled?"blur(12px)":"none",borderBottom:scrolled?"1px solid #1e2d4a":"1px solid transparent",transition:"all .3s",padding:"0 24px" }}>
        <div style={{ maxWidth:1100,margin:"0 auto",height:64,display:"flex",alignItems:"center",justifyContent:"space-between" }}>
          ...
          <div style={{ display:"flex",alignItems:"center",gap:12 }}>
            ...
            <button style={gBtn} onClick={()=>setShowBot(true)} aria-label="פתח מחשבון פיצויים חינמי">חשב פיצוי חינם</button>
          </div>
        </div>
      </header>
          ...
          <div style={{ maxWidth:1100,margin:"0 auto",padding:"80px 24px",width:"100%" }}>
            <div style={{ maxWidth:700 }}>
              ...
              <p style={{ fontSize:17,color:"#7a8fa5",lineHeight:1.8,marginBottom:36,maxWidth:530 }}>
                {MY_NAME} — {MY_TITLE} | מחשבון הפיצויים מעריך את שווי התיק שלך תוך דקות. שיחה ישירה, שכ"ט רק מהפיצוי.
              </p>
              <div style={{ display:"flex",gap:14,flexWrap:"wrap",marginBottom:48 }}>
                <button style={gBtn} onClick={()=>setShowBot(true)} aria-label="פתח מחשבון פיצויים">🤖 חשב פיצוי חינם</button>
                <button style={oBtn} onClick={()=>window.open(`https://wa.me/${WA}`,"_blank")} aria-label="פתח שיחת וואטסאפ">💬 וואטסאפ עכשיו</button>
              </div>
              <div className="g4" style={{ maxWidth:560 }} role="list" aria-label="נתונים עיקריים">
                {[["₪0","תשלום מראש"],["8%","שכ\"ט מינימלי"],["60","יום — פעל עכשיו"],["24h","מענה ראשוני"]].map(([n,l])=>(
                  <div key={l} role="listitem" style={{ textAlign:"center",padding:"16px 8px",background:"#0d1323",border:"1px solid #1e2d4a",borderRadius:14 }}>
                    <div style={{ fontSize:22,fontWeight:900,color:G,marginBottom:4 }}>{n}</div>
                    <div style={{ fontSize:11,color:"#7a8fa5",lineHeight:1.4 }}>{l}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <Reveal>
            <div style={{ maxWidth:500,margin:"0 auto",textAlign:"center" }}>
              ...
              <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
                <button style={{ ...gBtn,width:"100%",justifyContent:"center",fontSize:16,padding:16 }} onClick={()=>window.open(`https://wa.me/${WA}`,"_blank")} aria-label="שלח הודעת וואטסאפ">💬 שלח הודעת וואטסאפ</button>
                <a href={`tel:${PHONE}`}>
      ...
      {/* Cookie Banner */}
      {!cookie && (
        <div className="ck" role="dialog" aria-modal="false" aria-label="הסכמה לעוגיות">
          <p style={{ maxWidth:700,margin:0 }}>
            האתר משתמש בעוגיות לצרכי ניתוח ופרסום ממוקד. שיחות הבוט <strong>אינן נשמרות</strong> בשרתינו.{" "}
            <a href="/privacy" style={{ color:G }}>מדיניות פרטיות</a>
          </p>
          <div style={{ display:"flex",gap:8,flexShrink:0 }}>
            <button style={{ ...oBtn,padding:"8px 16px",fontSize:13 }} onClick={()=>setCookie(true)} aria-label="אשר עוגיות הכרחיות בלבד">רק הכרחיות</button>
            <button style={{ ...gBtn,padding:"8px 20px",fontSize:13 }} onClick={()=>setCookie(true)} aria-label="אשר את כל העוגיות">אישור הכל ✓</button>
          </div>
        </div>
      )}
