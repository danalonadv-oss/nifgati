...
function getPersonalizedOpening(pageSlug, utmTerm) {
  ...
  if (p === 'machshevon') {
    if (t.includes('כאב') && t.includes('סבל'))
      return 'שלום! ברוך הבא למחשבון כאב וסבל. אני אחשב לך בדיוק כמה מגיע לך לפי נוסחת פלת״ד 📊';
    if (t.includes('נזקי גוף') || t.includes('נזק גוף'))
      return 'שלום! ברוך הבא למחשבון נזקי גוף. ספר לי על הפגיעה ואחשב את הפיצוי שלך 📊';
    if (t.includes('מחשבון'))
      return 'שלום! מחשבון הפיצויים מוכן. ענה על 3 שאלות קצרות ותקבל הערכה מיידית 🧮';
  ...
  if (p === 'taonat-drakhim') {
    if (t.includes('דוגמאות'))
      return 'שלום! מחפש דוגמאות לפיצויים? ספר לי על המקרה שלך ואראה לך כמה קיבלו אחרים עם פציעה דומה 💰';
    if (t.includes('נזקי גוף'))
      return 'שלום! רוצה לבדוק כמה שווה תביעת נזקי הגוף שלך? אני אעריך את הסכום תוך דקה ⚖️';
    if (t.includes('עורך דין'))
      return 'שלום! לפני שבוחרים עורך דין — בוא נבדוק כמה באמת מגיע לך. חינם ואנונימי 🔒';
    if (t.includes('נפגעתי'))
      return 'שלום! נפגעת בתאונת דרכים? בוא נבדוק יחד כמה פיצוי מגיע לך — חינם תוך 60 שניות ⚡';
    if (t.includes('מה עושים') || t.includes('מה לעשות'))
      return 'שלום! אחרי תאונת דרכים חשוב לפעול נכון. ספר לי מה קרה ואדריך אותך + אחשב כמה מגיע לך 📋';
    if (t.includes('ביטוח חובה'))
      return 'שלום! רוצה להגיש תביעת ביטוח חובה? בוא נבדוק כמה מגיע לך לפי החוק ⚖️';
    if (t.includes('פיצוי') || t.includes('פיצויים'))
      return 'שלום! בוא נחשב כמה פיצוי מגיע לך על התאונה — חינם, אנונימי, תוך 60 שניות 💰';
    if (t.includes('תביעה'))
      return 'שלום! חושב על תביעה? בוא נבדוק קודם כמה מגיע לך — בלי התחייבות ⚡';
  ...
  if (p === 'avoda') {
    if (t.includes('דרך לעבודה'))
      return 'תאונה בדרך לעבודה? בוא נחשב כמה פיצוי מגיע לך לפי חוק הפלת״ד.';
  ...
  if (p === 'tzlipat-shot') {
    if (t.includes('דוגמאות'))
      return 'שלום! מחפש דוגמאות לפיצוי על צליפת שוט? ספר לי על המקרה שלך ואראה לך מקרים דומים 💰';
    if (t.includes('כאבי ראש'))
      return 'שלום! כאבי ראש אחרי תאונה מוכרים בחוק כצליפת שוט — בוא נחשב כמה מגיע לך 🧮';
    if (t.includes('צליפת שוט'))
      return 'שלום! צליפת שוט מזכה בפיצוי גם ללא שבר. בוא נחשב כמה מגיע לך — חינם ⚡';
...
export default function LandingPage({ pageTitle, pageSubtitle, heroEmoji, bullets, ctaText, pageSlug, bannerText, socialProofAmount, socialProofLabel, extraContent, heroExtra, metaTitle, metaDescription }) {
  ...
  useEffect(() => {
    ...
    const faqSchemas = {
      ...
      'machshevon': [
        { q: "איך עובד מחשבון כאב וסבל?", a: "המחשבון מבוסס על נוסחת הפלת״ד ומחשב פיצוי לפי אחוז נכות, ימי אשפוז וגיל הנפגע." },
        { q: "מה נכלל בחישוב הפיצוי?", a: "4 ראשי נזק: כאב וסבל, הפסד שכר, אובדן כושר השתכרות והוצאות רפואיות." },
      ...
      'tzlipat-shot': [
        ...
        { q: "כמה זמן לוקח להחלים מצליפת שוט?", a: "רוב המקרים — 3-6 חודשים. במקרים קשים הכאבים הופכים כרוניים ומזכים בנכות צמיתה." },
        { q: "האם כאבי ראש אחרי תאונה מזכים בפיצוי?", a: "כן. כאבי ראש מתמשכים הם סימפטום מוכר של צליפת שוט ומזכים בפיצוי לפי החוק." },
      ],
      'ptsd': [
      ...
      <main id="main-content" role="main" style={{ flex:1 }}>
        ...
        <section aria-label={dynamicTitle} style={{ display:"flex", alignItems:"center", position:"relative", overflow:"hidden", paddingTop:showBanner ? 120 : 80 }}>
          ...
          <div className="hero-wrap" style={{ maxWidth:900, margin:"0 auto", padding:"48px 24px 32px", width:"100%", textAlign:"center" }}>
              ...
              <h1 className="ht" style={{ fontSize:"clamp(32px, 5vw, 56px)", fontWeight:800, lineHeight:1.2, marginBottom:14 }}>
                {dynamicTitle}
              </h1>
              <h2 className="hero-sub" style={{ fontSize:"clamp(17px, 2.5vw, 22px)", fontWeight:500, color:"#1a1a2e", marginBottom:24, lineHeight:1.8 }}>{pageSubtitle}</h2>
